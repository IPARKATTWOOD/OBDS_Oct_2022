...
```{r}
seurat_after_qc <- readRDS("/project/obds/ipark/scRNAseq/Day2/seurat_after_qc.rds")
```
...
```{r}
# to reduce variability in each cluster to compare
seurat_after_qc <- SCTransform(seurat_after_qc, vars.to.regress = "percent_mt")
...
```{r}
VlnPlot(seurat_after_qc, features = c("nCount_RNA", "nCount_SCT"), same.y.lims = T)
VlnPlot(seurat_after_qc, features = c("nFeature_RNA", "nFeature_SCT"), same.y.lims = T)
```
...
```{r}
DefaultAssay(seurat_after_qc) <- "SCT" # default is SCT so no need here but if I want RNA I need to specify 'RNA' 
...
# use sct.pca not to overwirte the previous pca
```
...
seurat_after_qc <- FindClusters(seurat_after_qc, graph.name = "SCT_snn", resolution = 0.5, random.seed = 0)
```
...
#?table
#head(seurat_after_qc)
#seurat_after_qc[["RNA_snn_res.0.5"]]
#seurat_after_qc[["SCT_snn_res.0.5"]]
table(seurat_after_qc$RNA_snn_res.0.5, seurat_after_qc$SCT_snn_res.0.5)
# 0 means they are identical, the table shows there are some differences in RNA_snn and SCT_snn
```
...
```{r}
hto12_object <- CreateSeuratObject(hto12.umis.common, assay = "RNA", min.cells = 3, min.features = 200)
...
# 12 cell lines and 8533 barcodes
```
...
```{r}
hto12_object <- HTODemux(hto12_object, assay = "HTO")
```
...
# the title indicates correlation
```
what do you notice ?
...
```{r}
FeatureScatter(hto12_object, feature1 = "K562-A", feature2 = "KG1-A", group.by = "HTO_classification.global")
```
...
VlnPlot(hto12_object, features = "nCount_RNA", pt.size = 0)
VlnPlot(hto12_object, features = "nCount_HTO", pt.size = 0)
```
...
# on the right is doublets (multiple labels) and the far right is negative (no labels)
```
...
names(hto12_object[[]])
Idents(hto12_object) <- "HTO_classification.global"
hto12_object.subset <- subset(hto12_object, idents = "Negative", invert =T)
# throw away Negative
...
# negative ones won't be clustered - probably somewhere inbetween
```
...
```{r}
DimPlot(hto12_object.subset, reduction = "hto_umap", group.by = "hash.ID")
DimPlot(hto12_object.subset, reduction = "hto_umap", group.by = "hash.ID", label = T)
```
