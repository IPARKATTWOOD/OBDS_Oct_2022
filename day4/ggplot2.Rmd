...
```{r}
ggplot(diamonds, aes(x = carat, y = price)) +
geom_point(size = 0.7, aes(color = cut))
```
...
```{r}
# for a line; lm linear 
ggplot(ChickWeight, aes(x = Time, y = weight)) +
         geom_point(aes(colour = Diet)) +
         geom_smooth(method = lm)
# without method it shows default
ggplot(ChickWeight, aes(x = Time, y = weight)) +
         geom_point(aes(colour = Diet)) +
         geom_smooth()
# individal lines
...
# adjust size fig.height=6, fig.width=6 depending on how it looks.
...
```{r}
ggsave("superplot.pdf", second_row)
# images will be saved in 7X7 in default
