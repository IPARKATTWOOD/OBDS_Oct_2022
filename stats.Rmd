...
```{r}
summary(x)
# min 0% - 1st Qu 25% - median 50% - 3rd Qu 75% - max 100% 
```
...
```{r}
# Quantile (divide by 4)
# Quintile (divide by 5)
# decile (divide by 10)
# Percentile (divide by 100)
y <- quantile(x, seq(0, 1, 0.1)); y
```

- Visualise the distribution of those values as a histogram.

```{r}
# ggplot is for dataframe
hist(x, breaks = 20)
```
...
```{r}
# baseR function hist() doesn't have the aes functions. 
hist(x, breaks = 20)  
abline(v=mean(x), col="red")
abline(v=median(x), col="red", lyt=2)
...
```{r}
set.seed(1)
a <- rnorm(n=1000000, mean=10, sd=5)
hist(a, breaks=100)
...
a <- seq(-5, 5, 0.1)
b <- data.frame(a, cdf = pnorm(a))
b
ggplot(b) +
  geom_point(aes(x= a, y= cdf))

b <- seq(from = -5, to = 5, by = 0.1)
norm.dot <- data.frame(x = b, cdf = pnorm(b))
ggplot(norm.dot) +
...
norm.dot <- data.frame(x = b, cdf = qnorm(b))
ggplot(norm.dot) +
  geom_point(aes(x = b, y = cdf))
```
...
```{r}
a <- seq(-5, 5, 0.1)
b <- data.frame(a, cdf = dnorm(a))
ggplot(b) + 
  geom_point(aes(x=a, y=cdf))


b <- seq(from = -5, to = 5, by = 0.1)
norm.dot <- data.frame(x = b, cdf = dnorm(b))
ggplot(norm.dot) +
  geom_point(aes(x = b, y = cdf))
```
...
probofGT2 <- 1-pnorm(2); probofGT2
```

- What is the probability of observing a value between -2 and 2?

```{r}
probofbwpm2 <- pnorm(2) - pnorm(-2); probofbwpm2
# or
1- probofGT2*2
```
...
logcounts_matrix <- read_xlsx("/project/obds/shared/resources/3_r_stats_genomics/statistics/data/GSE111543.xlsx", "logcounts")
logcounts_matrix
...
cell_info <- read_xlsx("/project/obds/shared/resources/3_r_stats_genomics/statistics/data/GSE111543.xlsx", "cell_info")
...
```{r}
# can use tidyverse (difficult) or two separate matrices using baseR (simple)
# baseR
logcounts_matrix
cell_info
...
gene1_mock <- logcounts_matrix[1, cells_mock]
gene1_infected <- logcounts_matrix[1, cells_infected]

result <- t.test(gene1_mock, gene1_infected)
result$p.value
summary(result)
```
...
# my answers - wrong
#t_test_row <- function(cells_mock, cells_infected) {
 for(x in 1:nrow(logcounts_matrix)) +
  cells_mock <- cell_info$Sample[cell_info$Infection == "Mock"] +
  cells_infected <- cell_info$Sample[cell_info$Infection == "STM-LT2"] +
  gene_mock <- logcounts_matrix[x, cells_mock] +
  gene_infected <- logcounts_matrix[x, cells_infected] +
...
t_test_row <- function(index, cells_mock, cells_infected) {
  cells_mock <- cell_info$Sample[cell_info$Infection == "Mock"]
  cells_infected <- cell_info$Sample[cell_info$Infection == "STM-LT2"]
  gene_mock <- logcounts_matrix[index, cells_mock]
  gene_infected <- logcounts_matrix[index, cells_infected]
  result <- t.test(gene_mock, gene_infected)
  return(result$p.value)
}
t_test_row (2, cells_mock, cells_infected)
...
```{r}
excel_sheets("/project/obds/shared/resources/3_r_stats_genomics/statistics/data/GSE111543.xlsx")
gene_table <- read_excel("/project/obds/shared/resources/3_r_stats_genomics/statistics/data/GSE111543.xlsx", "gene_info")
gene_table
