...
```{r}
BiocManager::valid()
# do not install on this server - ask genmail@...
```
...
```{r}
IlluminaQuality(0:40) #the bigger the vaule, the higher the quality
```
...
# the links from the command above doesn't work on the server but works in personal computer
```
...
```{r}
# read fasta files
## readBStringSet("/project/obds/shared/resources/3_r_stats_genomics/r_bioconductor/TruSeq3-PE-2.fa")

file.edit("/project/obds/shared/resources/3_r_stats_genomics/r_bioconductor/TruSeq3-PE-2.fa")
readDNAStringSet("/project/obds/shared/resources/3_r_stats_genomics/r_bioconductor/TruSeq3-PE-2.fa")
...
# "alt" = alternative seq due to multiple seq
```
...
```{r}
Ebox <- "CANNTG"
#countPattern("CANNTG", chrY)
countPattern(Ebox, chrY, fixed = F)
# fixed = F means N will be interepreted as any base, T means there must be an N in the genome at that place.
```
...
#include=TRUE can be removed not necessary here
```
...
```{r}
human_symbols <- head(keys(org.Hs.eg.db, "SYMBOL")); human_symbols
human_symbols_10 <- keys(org.Hs.eg.db, "SYMBOL")[1:10]; human_symbols
# human_symbols <- head(keys(org.Hs.eg.db, "SYMBOL"), n=8)
# human_symbols <- keys(org.Hs.eg.db, "SYMBOL")[1:8]; human_symbols
```
...
select(org.Hs.eg.db, human_symbols, columns = c("ENTREZID", "ENSEMBL"), keytype = "SYMBOL")

AnnotationDbi::select(org.Hs.eg.db, human_symbols_10, columns = c("ENTREZID", "ENSEMBL"), keytype = "SYMBOL")

AnnotationDbi::select(org.Hs.eg.db, keys(org.Hs.eg.db, "SYMBOL"), columns = c("ENTREZID", "ENSEMBL"), keytype = "SYMBOL")

```

  + What happens if you do not specify `AnnotationDbi`?

> Answer: it will work if the package is loaded
> 
...
#mapIds doesn't give you the output as a table but a vector. so select() is better. 
mapIds(org.Hs.eg.db, human_symbols, "ENSEMBL", "SYMBOL", multiVals = "list")
# but can add miltiVals = "list" for better visualisation.
```
...
list1 <-listDatasets(mart); list1

view(list1)
# or click list1 object on Environment
```
...
```{r}
list2 <- listAttributes(mart); list2
...
getBM(attributes =c("hgnc_symbol", "chromosome_name", "start_position", "end_position", "strand"), filters= "hgnc_symbol", values=human_symbols, mart)
```
...
colData(demo_se)
rowData(demo_se)
mcols(demo_se) # not needed in this dataset
metadata(demo_se) # not specified
```
...
head(assay_counts, 10)

rownames(assay_counts) <- assay_counts$X
assay_counts$X <- NULL # remove X column
