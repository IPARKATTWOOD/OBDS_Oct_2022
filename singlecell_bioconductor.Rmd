...
```{r}
library(DropletUtils)
#sce <- read10xCounts("/project/obds/shared/resources/4_r_single_cell/singlecell_bioconductor/filtered_feature_bc_matrix")
# to give the name to the file use c()
sce <- read10xCounts(c(pbmc5k="/project/obds/shared/resources/4_r_single_cell/singlecell_bioconductor/filtered_feature_bc_matrix"))
```
...
```{r}
# to add a column that indicates which is mito genes.
library(scuttle) # it may have been loaded as part of one of the packages
?grep
#rowData(sce)$Symbol
is.mito <- grep("^MT-", rowData(sce)$Symbol); is.mito
sce <- scuttle::addPerCellQC(sce, subset =list("is.mito" =is.mito))
# sum is total depth of reads across genes
colData(sce)
```
...
# plot 1: >4500, plot 2: >1500 , plot 3: cut off >15% 
```
...
```{r}
# to filter the data 
#colData(sce)$sum > 4500 & colData(sce)$subsets_is.mito_percent < 15 & colData(sce)$detected >1500
sce <- sce[   , colData(sce)$sum > 4500 & colData(sce)$subsets_is.mito_percent < 15 & colData(sce)$detected >1500]
sce
# sce after filtering discards the unwanted info. so rename if want to keep
```
...
```{r}
# to add means and detected of each row
# Not sure why this step was for
...
rowData(sce) %>%
    as_tibble() %>% 
    ggplot() +
    geom_point(aes(x=mean, y=detected)) +
...
library(DelayedMatrixStats)
# 
x <- DelayedArray(assay(sce, "counts")) # take out only counts from the big obj sce
plot_data <- tibble(
...
#highly viable genes are of our interest - logcounts plot shows a better separation
```
...
> Answer: don't use this function unless you understand the data well. can be used when you have a strong dataset/cluster effect; don't change sizes,
> 
> 
...
dec
#block = batch, design = can initially remove variables e.g. batch if you are interested in the data by treatment (design = ~Batch)
class(dec)
#Dframe or DataFrame = not data.frame
# pvalue of bio variance <=0
```
...
# each dot = gene, total variance (black) - bio v (blue) = tech variance (red)  ?
```
...
## ggplot2
ggplot() +
    geom_point(aes(mean, bio), data = 
                 as.data.frame(dec)[!rownames(dec)%in%hvg ,],  color = "black") +
    geom_point(aes(mean, bio), data = as.data.frame(dec)[hvg,], color = "blue") 
# 10% hvg and 90% non-hvg, bio = biological variance
# as.tibble works well but it will change the return as tibble. since tibble doesn't have rownames, it makes sense to use as.data.frame.
```
...
# include name not to overwrite the original pca
```
...
```{r}
reducedDim(sce_denoise, "PCA_denoise") # to get the dim of the PCA_denoise
set.seed(1234)
...
```{r, message=FALSE}
g <-    scran::buildKNNGraph(sce_denoise, use.dimred = "PCA_denoise")
#igraph::cluster_louvain(g)
#str(igraph::cluster_louvain(g))
colData(sce_denoise)[["cluster_louvain"]] <- factor(igraph::cluster_louvain(g)$membership)
colData(sce_denoise)
```
...
for (d in c(5, 10, 13, 15)) {
    g <- scran::buildSNNGraph(t(reducedDim(sce_denoise, "PCA")), d = d)
    colData(sce)[[sprintf("snn_d", d)]] <- factor(igraph::cluster_louvain(g)$membership)
    gg_d <- reducedDim(x = sce_denoise, type = "UMAP") %>%
        as.data.frame() %>%
        as_tibble() %>%
...
```{r}
sce_denoise$quickCluster <- scran::quickCluster(sce_denoise   )

gg_cluster <- reducedDim(x = sce_denoise, type = "UMAP") %>%
    as.data.frame() %>%
    as_tibble() %>%
    bind_cols(colData(sce_denoise) %>% as_tibble()) %>%
    sample_frac() %>%
...
```{r}
markers <- scran::findMarkers(sce_denoise, groups = sce_denoise$cluster_louvain, test.type = "wilcox")   
#markers <- scran::findMarkers(sce_denoise, groups = sce_denoise$cluster_louvain, test.type = "wilcox", lfc = 1, direction = "up")   # to get stringent markers which are +
class(markers)
markers[[1]]
```
...
```{r}
gg_marker <- reducedDim(x = sce_denoise, type = "UMAP") %>%
    as.data.frame() %>%
    as_tibble() %>%
    mutate(marker = assay(sce_denoise, "logcounts")[marker_id, ]) %>%
  slice_sample(prop = 1) %>% # gives you clusters in a random order?
    ggplot() +
...
```{r}
# it doesn't open on this server !!!!!!!!!
library(iSEE)
#panelDefaults(downsamples = T) # optional if data is big
app <- iSEE(sce)
if (interactive()) {
  shiny::runApp(app)
}
