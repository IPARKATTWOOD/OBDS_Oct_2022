...
# data.frame also creates as but as.data.frame also works
```
...
```{r}
pca_iris_species <- ggplot(pca_iris_dataframe, aes(x= PC1, y= PC4, color = Species)) +
  geom_point()
...
```{r}
# pick the most variable = Petal.Length, add the data
pca_iris_dataframe <-  cbind(pca_iris_dataframe, Petal.Length = iris$Petal.Length)
head(pca_iris_dataframe)
```

```{r}
ggplot(pca_iris_dataframe, aes(x= PC1, y= PC2, color = Petal.Length)) +
  geom_point()

ggplot(pca_iris_dataframe, aes(x= PC1, y= PC2, size = Petal.Length, color = Petal.Length, shape = Species)) +
  geom_point()
```
...
#names(explained_variance_ratio) <- paste0("PC", 1:length(explained_variance_ratio)) # alternative - automated
...
# geom_bar: only x, geom_col: both x and y should be given
```
...
set.seed(1) # Set a seed for reproducible results
tsne_iris <- Rtsne(pca_iris_dataframe, dims = 2, initial_dims = 50, perplexity = 30, theta = 0.5)
str(tsne_iris)
```
...
```{r}
tsne_iris_dataframe <- data.frame(pca_iris, )
...
```{r}
dist_iris <- dist(iris_features, method = "euclidean"); dist_iris
...
iris_clusters_dataframe$hclust_average <- as.factor(iris_hclust_complete_labels)
...
plot_ward <- ggplot(iris_clusters_dataframe, aes(x= Sepal.Length, y= Sepal.Width, color = hclust_ward)) + geom_point(size = 5)
...
```{r}
dbscan_plot <- ggplot(iris_clusters_dataframe, aes(x=Sepal.Length, y= Sepal.Width, color= dbscan)) + geom_point(size=3)
...
```{r}
hdbscan_iris <- hdbscan(iris_features, minPts = 5, gen_hdbscan_tree = FALSE, gen_simplified_tree = FALSE)
hdbscan_iris
# minPts minimum size of clusters - any group of cells less than 5 will be discarded?
```
...
```{r}
hdbscan_plot <- ggplot(iris_clusters_dataframe, aes(x= Sepal.Length, y= Sepal.Width, color = hdbscan)) + geom_point(size=3)
...
ggplots_dbscan <- cowplot::plot_grid(dbscan_plot, hdbscan_plot)
ggsave(filename = "ggplots_dbscan.png", dpi = 300)
```
...
```{r}
set.seed(1) # Set a seed for reproducible results
kmeans_iris <- kmeans(iris_clusters_dataframe, centers =    )
kmeans_iris
```
