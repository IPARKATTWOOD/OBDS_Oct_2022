...
```{r}
# if no messages, it has already been loaded.
library(tidyverse)
```
...
```{r, eval=FALSE}
file.edit("iris.tsv")
# can use file.edit function or just click the file
```
...
tibble(x = 1:5, y = 1L, z = as.integer(x ^ 2 + y))
...
iris_raw %>% group_by(species) %>% 
  summarise(Sepal_Length.mean = mean(sepal_length), 
            Sepal_Length.median = median(sepal_length),
            Sepal_Length.minimum = min(sepal_length), 
            Sepal_Length.maximum = max(sepal_length)
            )
...
```{r}
# Copy the code chunk above and extend with more pipes
named_vector <- iris_raw %>% group_by(species) %>% 
  summarise(across(where(is.numeric), mean)) %>% 
  filter(sepal_length < 6) %>% 
  arrange(desc(sepal_length)) %>% 
...
```{r}
iris_raw %>% mutate(sepal_area = sepal_length*sepal_width) %>% slice_head(n=6)
```
...
iris_long <- iris_raw %>% 
  pivot_longer(!species, names_to = "Variable measured", values_to = "Value")
...
iris_long2 <- iris_raw %>%
  ...
  select(!c(ID, species)) %>% 
  pivot_longer(!species_ID, names_to = "Variable measured", values_to = "Value")
...
iris_wide <- iris_long2 %>% 
  pivot_wider(names_from = "Variable measured", values_from = "Value")
...
```{r}
ggplot(exprs_assay_long, aes(x = gene_expression)) +
  geom_histogram() +
  facet_wrap(~gene)
# don't make spacing or use ''
```
...
# no need to give y since sample_info is already specified
# the order of name should match with the order of the data
```
